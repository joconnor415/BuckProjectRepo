...
#get a list of network and write to file
def get_network_list(data=None, organism=None, query=None, fileName=None):
    '''
    ...
    if fileName is None: fileName = "network.txt"
    ...
    networks = []
    if p is not None:
        networks = sorted([network for network in p.split('\n')])
        f = open(fileName, "w+")
...
network_file = get_network_list("gmdata-2012-08-02", "S. Cerevisiae", "yeast.query", "networks.txt")
networks    = getNetworks(network_file)
